...
def prepare_climate_file ():
    ...
    start_datetime = df_solar.index[0].replace(tzinfo=None)  
    end_datetime =df_solar.index[-1].replace(tzinfo=None)
    ...
    #gets daily GHI by summing the resampled values and then converts Watt to MJ
    et_daily["net_rad(MJ/m2)"] = df_solar.resample('D')['poa_global'].sum() * 1e-6

    #gets daily air temperature by performing operation on the resampled values

    et_daily["t_mean(K)"] = (df_solar.resample('D')['temp_air'].mean()).apply(celsius2kelvin) #mean daily temperature in K

    et_daily["t_mean(C)"] = (df_solar.resample('D')['temp_air'].mean()) #mean daily temperature in C

    et_daily["t_max(C)"] = (df_solar.resample('D')['temp_air'].max()) #maximum daily temperature in C

    et_daily["t_min(C)"]= (df_solar.resample('D')['temp_air'].min()) #minimum daily temperature in C
    
    #gets daily mean wind speed by averaging the resampled values (unknown height of measurement, so not corrected)
    et_daily["ws(m/s)"] = df_solar.resample('D')['wind_speed'].mean()
